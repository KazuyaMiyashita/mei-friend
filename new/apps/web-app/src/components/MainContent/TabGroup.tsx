...
// Same facsimile icon as in the header (SVG inline)
...
function TabLabel({ instance }: { instance: PanelInstance }) {
  const mf = useMeiFriend(instance.meiFriendId)
  const fileName = mf?.core?.fileName ?? instance.meiFriendId ?? '—'
...
function SortableTab({
  ...
  const mf = useMeiFriend(instance.meiFriendId)
  const isDirty = mf?.core?.isDirty ?? false
  ...
  return (
    // biome-ignore lint/a11y/useSemanticElements: Contains a close button, so it cannot be a native button
    <div
      ref={ref}
...
export default function TabGroup({
  ...
  // Tab bar itself is also a drop target (equivalent to center)
  ...
  return (
    ...
    >
      ...
      {/* Context menu (portal-less, position: fixed) */}
      {contextMenu && (
        <ContextMenu
          x={contextMenu.x}
          y={contextMenu.y}
