...
export default function MenuBar({
  ...
  const { meiFriendId, instance: focusedInstance } = useFocusedMeiFriend()
  const { workspace, saveMei } = useMeiFriendWorkspace()
  const snapshot = useWorkspaceSnapshot()

  // Helper for displaying shortcuts (uses defaults only; customization via SettingsPanel)
  ...
  const handleRenameWorkspace = async () => {
    ...
    if (newName !== null) {
      ...
      const success = await workspace.runApi((api) => api.localPersistence.saveWorkspaceConfig())
      if (success) {
    ...
    <nav className="navbar" id="menuBar">
      ...
      >
        ...
        {item(
          'Save SVG',
          undefined,
          focusedInstance ? () => focusedInstance.runApi((api) => api.fileIO.saveSvg()) : undefined,
        )}
      </Dropdown>
      ...
      >
        {item(
          ...
          focusedInstance ? () => focusedInstance.runApi((api) => api.history.undo()) : undefined,
        )}
        {item(
          'Redo',
          getShortcut('document.redo'),
          focusedInstance ? () => focusedInstance.runApi((api) => api.history.redo()) : undefined,
        )}
        <MenuLine />
        {item('Search', '⌘F')}
        {item('Replace', '⌥⌘F')}
      ...
      >
        {item(
          ...
          '⌫',
          focusedInstance
            ? () => focusedInstance.runApi((api) => api.meiEditor.deleteSelectedElements())
            : undefined,
        )}
      </Dropdown>
