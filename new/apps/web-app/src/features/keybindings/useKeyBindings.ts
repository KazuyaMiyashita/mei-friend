import { findNoteSelectionById, navigateAddressByOffset } from '@mei-friend/core'
import type { SelectionState, VerovioState } from '@mei-friend/plugins'
  ...
  useEffect(() => {
    const handler = async (e: KeyboardEvent) => {
      ...
      const instance: AppMeiFriend | null = meiFriendId
        ? ((ws.getMeiFriendInstance(meiFriendId) as AppMeiFriend) ?? null)
        : null
      if (!instance) return

      const selState = instance.getSnapshot().selection as SelectionState | undefined
      ...
      if (isInputActive && noteSel) {
        ...
        if (!isCmd(e) && !e.altKey && !e.shiftKey && /^[a-g]$/.test(key)) {
          ...
          if (newId) {
            const doc = (await instance.runApi((api) => api.core.getDocument()))?.getDocument()
            if (doc) {
        ...
        if (!isCmd(e) && !e.altKey && !e.shiftKey && e.key.startsWith('Arrow')) {
          e.preventDefault()
          const vrvState = instance.getSnapshot().verovio as VerovioState | undefined
