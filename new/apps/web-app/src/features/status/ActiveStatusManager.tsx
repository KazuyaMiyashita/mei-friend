import { getActiveMeter, getDurationAndOffset, getElementById } from '@mei-friend/core'
...
export function ActiveStatusManager() {
  ...
  const mf = useMeiFriend(focusedId)
  const workspace = useWorkspace()
  const { setStatusText } = useStatus()

  // biome-ignore lint/correctness/useExhaustiveDependencies: update status on XML change (e.g. undo)
  useEffect(() => {
    if (!focusedId) {
      setStatusText('Ready.')
      return
    ...
    updateStatus()
  }, [focusedId, mf?.selection?.selection, mf?.core?.xmlContent, workspace, setStatusText])
