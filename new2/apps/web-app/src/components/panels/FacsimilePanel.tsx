import { resolveWorkspacePath } from '@mei-friend2/core'
import type { FacsimileZone } from '@mei-friend2/plugins'
...
function applyResize(
  ...
  if (handle === 'move') return { ulx: ulx + dx, uly: uly + dy, lrx: lrx + dx, lry: lry + dy }
...
export default function FacsimilePanel({ meiFriendId }: Props) {
  ...
  const fileName = useMeiFriend(meiFriendId, (s) => s.fileName)
  const hasFacsimile = useMeiFriend(meiFriendId, (s) => s.facsimile.hasFacsimile)
  const surfaces = useMeiFriend(meiFriendId, (s) => s.facsimile.surfaces)
  const zones = useMeiFriend(meiFriendId, (s) => s.facsimile.zones)
  const currentSurfaceIndex = useMeiFriend(meiFriendId, (s) => s.facsimile.currentSurfaceIndex)
  const zoomPercent = useMeiFriend(meiFriendId, (s) => s.facsimile.zoomPercent)
  const showZones = useMeiFriend(meiFriendId, (s) => s.facsimile.showZones)
  const showTitles = useMeiFriend(meiFriendId, (s) => s.facsimile.showTitles)
  const editMode = useMeiFriend(meiFriendId, (s) => s.facsimile.editMode)
  const selectedZoneId = useMeiFriend(meiFriendId, (s) => s.facsimile.selectedZoneId)
  ...
  useEffect(() => {
    if (!hasFacsimile || !surfaces) return
    let cancelled = false

    const loadImages = async () => {
      for (const surface of surfaces) {
        if (!surface.target || imageUrls.has(surface.id) || imageErrors.has(surface.id)) continue
        ...
        try {
          ...
          setImageUrls((prev) => new Map(prev).set(surface.id, url))
        } catch {
          if (cancelled) return
          const resolved = fileName
            ? resolveWorkspacePath(fileName, surface.target)
            : surface.target
          setImageErrors((prev) => new Map(prev).set(surface.id, resolved))
        } finally {
          if (!cancelled)
            setLoadingIds((prev) => {
  ...
  }, [surfaces, fileName, workspace, hasFacsimile, imageUrls, imageErrors])
  ...
  useEffect(() => {
    ...
    const onMouseUp = async () => {
      ...
      if (liveZone) {
        instance?.runApi((api) =>
          api.facsimile.updateZoneCoords(
            rs.zoneId,
            liveZone.ulx,
            liveZone.uly,
            liveZone.lrx,
  ...
  const onViewportMouseDown = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      if (!editMode || resizeRef.current || currentSurfaceIndex == null) return
    ...
    [editMode, currentSurfaceIndex, surfaces],
  )
  ...
  useEffect(() => {
    ...
    const onMouseUp = async (e: MouseEvent) => {
      ...
      if (x1 - x0 > 4 && y1 - y0 > 4 && zoomPercent) {
        const zoom = (zoomPercent ?? 100) / 100
        instance.runApi((api) =>
  ...
  const surface = currentSurfaceIndex != null ? surfaces?.[currentSurfaceIndex] : null
  ...
  const surfaceZones = surface
    ? surface.zoneIds.flatMap((id) => {
        const z = zones?.get(id)
        return z ? [z] : []
      })
    : []
  const imgNaturalWidth = surface?.width ?? 1000
  const imgNaturalHeight = surface?.height ?? 1000
  ...
  return (
    <div className="facsimilePanel">
      <div className="facsimileControls">
        <label className="ctrlLabel" title="Zoom">
          ...
          <input
            ...
            onChange={(e) =>
              instance?.runApi((api) => api.facsimile.setZoom(Number(e.target.value)))
            }
            className="ctrlRange"
          />
        ...
        <div className="facsimilePageNav">
          <button
            ...
            onClick={() =>
              instance?.runApi((api) =>
                api.facsimile.setCurrentSurface((currentSurfaceIndex ?? 0) - 1),
              )
            }
            disabled={(currentSurfaceIndex ?? 0) <= 0}
            title="Previous surface"
          >
            ‹
          </button>
          <span className="facsimilePageInfo">
            {(currentSurfaceIndex ?? 0) + 1} / {surfaces?.length}
          </span>
          <button
            type="button"
            className="ctrlBtn"
            ...
            disabled={(currentSurfaceIndex ?? 0) >= (surfaces?.length ?? 1) - 1}
            title="Next surface"
          >
            ›
          </button>
        ...
        <button
          ...
          onClick={() => instance?.runApi((api) => api.facsimile.setShowZones(!showZones))}
          title="Show zones"
        >
          Zones
        </button>
        <button
          ...
          onClick={() => instance?.runApi((api) => api.facsimile.setShowTitles(!showTitles))}
        ...
        <button
          ...
          onClick={() => instance?.runApi((api) => api.facsimile.setEditMode(!editMode))}
        ...
        {editMode && selectedZoneId && (
          <button
            ...
            onClick={() => {
              instance?.runApi((api) => {
                api.facsimile.deleteZone(selectedZoneId)
                api.facsimile.selectZone(null)
              })
            }}
      ...
      {/* biome-ignore lint/a11y/useSemanticElements: original behavior */}
      <div
        ref={viewportRef}
      ...
      >
        {!surface ? (
          ...
          <div className="facsimileImageError">
            ...
            <p>Add the image to the workspace then reopen this file.</p>
          </div>
        ) : !imageUrl ? (
          <div className="facsimilePlaceholder">
          ...
          <div className="facsimileInner" style={{ transform: `scale(${zoom})` }}>
            ...
            {showZones && (
              <svg
                ...
                style={{ width: imgNaturalWidth ?? '100%', height: imgNaturalHeight ?? '100%' }}
                viewBox={
