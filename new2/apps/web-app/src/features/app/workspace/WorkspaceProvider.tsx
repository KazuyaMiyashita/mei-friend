...
import { MeiFriendWorkspace } from '@mei-friend2/core'
...
import {
  type AppWorkspace,
  createAppMeiFriendServices,
  createAppMeiFriendSlices,
  createAppWorkspaceSlices,
} from './plugins'
...
export function createWorkspace(localPersistenceEnabled = false): AppWorkspace {
  const ws = new MeiFriendWorkspace(
    `ws-${Date.now()}`,
    createAppWorkspaceSlices(localPersistenceEnabled),
    createAppMeiFriendSlices,
    [],
    createAppMeiFriendServices,
  )
  ws.initialize()
  return ws as unknown as AppWorkspace
}
...
export function WorkspaceProvider({ children }: { children: React.ReactNode }) {
  ...
  // Sync settings with workspace slice state
  useEffect(() => {
